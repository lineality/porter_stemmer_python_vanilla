...
https://tartarus.org/martin/PorterStemmer/c_thread_safe.txt


Classes:
    PorterStemmer: Main class implementing the Porter Stemming Algorithm

Functions:
    stem_file: Process a text file and output stemmed version
...
This module provides a pure Python implementation of the Porter Stemming Algorithm,
following Martin Porter's recommended extensions. The implementation requires no
external dependencies beyond Python's standard library.

The Porter Stemmer reduces words to their root form by removing common suffixes.
...
'''
NLKT code 
from https://www.nltk.org/_modules/nltk/stem/porter.html#PorterStemmer
...
__all__ = ['PorterStemmer', 'stem_file', 'run_comprehensive_tests']
...
class PorterStemmer:
    ...
    Attributes:
        to_lowercase (bool): Whether to convert words to lowercase before stemming
        preserve_original_on_error (bool): Whether to return original word on errors
    ...
    def __init__(
        ...
        Args:
            to_lowercase (bool): If True, convert words to lowercase before stemming.
                                Default is True.
            preserve_original_on_error (bool): If True, return original word when
    ...
    def stem_word(self, word: str) -> str:
        ...
        Args:
            word (str): The word to stem. Should contain only alphabetic characters.
        ...
        try:
            ...
            if self.preserve_original_on_error:
                ...
                raise RuntimeError(f"Error stemming word '{original_word}': {str(e)}") from e

    def stem_document(self, text: str) -> str:
        """
        Stem all words in a document while preserving formatting and punctuation.
        ...
        def replace_word(match):
            ...
            word = match.group(0)
            # Only stem if the word is purely alphabetic
            if word.isalpha():
                try:
                ...
                # Return non-alphabetic matches unchanged
                return word
    ...
    def stem_tokens(self, tokens: List[str]) -> List[str]:
        ...
        for i, token in enumerate(tokens):
            if not isinstance(token, str):
                raise TypeError(f"Token at index {i} must be a string, got {type(token).__name__}")
            ...
            try:
                ...
                if self.preserve_original_on_error:
                    ...
                    raise RuntimeError(f"Error stemming token at index {i}: {str(e)}") from e
    ...
    def _apply_case_pattern(self, original_word: str, stemmed_word: str) -> str:
        ...
        Character Requirements:
            This method enforces that the original word must contain ONLY alphabetic
            characters (A-Z, a-z). No punctuation, digits, or special characters are
            allowed. This constraint ensures consistent behavior with the rest of the
            Porter Stemmer implementation, which is designed for pure alphabetic text.
            
            Words containing apostrophes (like "don't"), hyphens (like "pre-process"),
            or any other non-alphabetic characters must be pre-processed before being
        ...
        Examples:
            >>> stemmer = PorterStemmer(to_lowercase=False)
    ...
    def _is_consonant(self, word: str, index: int) -> bool:
        ...
        if index < 0:
            raise IndexError(
                f"index {index} is negative. Must be >= 0"
            )
    ...
    def _ends_cvc(self, word: str) -> bool:
        ...
        # Check the pattern
        if (self._is_consonant(word, len(word) - 3) and
    ...
    def _step1c(self, word: str) -> str:
        ...
        if self.mode == "NLTK_EXTENSIONS":
            # NLTK rule: Y -> I only if preceded by consonant and stem length > 1
            # This means checking if the last character of the stem is a consonant
    ...
    def stem_file_lines(self, file_path, encoding='utf-8', errors='strict'):
        """
        ...
        try:
            with open(file_path, 'r', encoding=encoding, errors=errors, 
                    buffering=8192) as file_handle:
    ...
    def stem_file(
        ...
        This function processes files line-by-line using the stem_file_lines generator
        to handle large files efficiently. It can output to stdout, a file, or both,
        and provides progress indication for large files.
        
        The function uses the PorterStemmer.stem_file_lines() generator internally,
        which provides memory-efficient processing by yielding one line at a time
        ...
        Example:
            ...
            stem_file('document.txt')
            
            # Save to file with case preservation  
            stem_file('document.txt', preserve_case=True, output_file='stemmed.txt')
            
            # Process without progress indicator for automated scripts
            stem_file('large_file.txt', show_progress=False)
            
            # Process with specific output file
            stem_file('input.txt', output_file='output.txt')
        ...
        if not os.access(filename, os.R_OK):
            print(f"Error: Cannot read file '{filename}'. Permission denied.", file=sys.stderr)
            sys.exit(1)
        ...
        if output_file:
            ...
            try:
                ...
                print(f"Error: Cannot write to '{output_file}'. Permission denied.", file=sys.stderr)
                sys.exit(1)
            except Exception as e:
        ...
        stemmer_instance = PorterStemmer(to_lowercase=not preserve_case)
        ...
        try:
            ...
            for line_number, stemmed_line in enumerate(
                self.stem_file_lines(filename, encoding='utf-8', errors='strict'), 
                start=1
            ):
                try:
                    ...
                    if show_progress and file_size > 1024 * 1024:  # Only for files > 1MB
                        if (bytes_processed - last_progress_update > 1024 * 1024 or 
                            lines_processed % 1000 == 0):
                            
                            # Estimate progress based on processed bytes
                            # This is approximate since stemmed size may differ from original
                            progress_percent = min(100.0, (bytes_processed / file_size) * 100)
                    ...
                    print(f"\nError processing line {line_number}: {e}", 
                        file=sys.stderr)
            ...
            print(f"  Bytes processed: {bytes_processed:,} (estimated)", file=sys.stderr)
            ...
            print(f"\nError: File '{filename}' not found or was deleted during processing.", 
                file=sys.stderr)
            sys.exit(1)
            ...
            print("Try using a different encoding or check file format.", file=sys.stderr)
            sys.exit(1)
            ...
            print(f"Processed {lines_processed:,} lines before interruption.", file=sys.stderr)
            if output_file and output_handle:
                print(f"Partial output may be available in: {output_file}", file=sys.stderr)
            sys.exit(1)
            
        except MemoryError:
            # This shouldn't happen with line-by-line processing, but just in case
...
def run_comprehensive_tests():
    ...
    # Summary
    all_passed = original_passed and nltk_passed and error_handling_passed and misc_features_passed
    ...
    print(f"Miscellaneous Features Tests: {'PASSED' if misc_features_passed else 'FAILED'}")
...
def test_original_mode():
    ...
    stemmer = PorterStemmer(mode="ORIGINAL")
    ...
    test_case("BLI -> BLE", "conformabli", "conform")  # CORRECTED: Original mode now uses BLI->BLE
    ...
    test_case("NOT special", "dying", "dy")  # CORRECTED: Step1b removes 'ing', no vowel in 'd' so Y stays
    test_case("NOT special", "lying", "ly")  # CORRECTED: Step1b removes 'ing', no vowel in 'l' so Y stays
    test_case("NOT special", "tying", "ty")  # Step1b removes 'ing', no vowel in 't' so Y stays
    ...
    if failed_tests:
        ...
        for fail in failed_tests[:5]:
            print(f"  - {fail['word']}: expected '{fail['expected']}', got '{fail['actual']}'")
...
def test_nltk_extensions_mode():
    ...
    test_case("Y stays (after vowel)", "say", "say")  # CORRECTED: NLTK doesn't change Y after vowel
    test_case("Y stays (after vowel)", "enjoy", "enjoy")  # CORRECTED: NLTK doesn't change Y after vowel
...
def test_miscellaneous_features():
    ...
    case_stemmer = PorterStemmer(to_lowercase=False)
...
# note: this assumes porter_module.py is the file name

# import from module
from porter_module import PorterStemmer

# Create a stemmer instance with default settings (converts to lowercase)
stemmer = PorterStemmer()

# Create a stemmer that preserves original case
case_preserving_stemmer = PorterStemmer(to_lowercase=False)
...
Examples:
  ...
  python porter_stemmer.py --file document.txt --preserve-case
    ...
    def run_live_demo(preserve_case=False):
        ...
        Raises:
            Exception: Any exceptions from the PorterStemmer are caught and
        ...
        try:
            ...
            lowercase_stemmer = PorterStemmer(to_lowercase=True)
            case_preserving_stemmer = PorterStemmer(to_lowercase=False)
            ...
            demo_stemmer = case_preserving_stemmer if preserve_case else lowercase_stemmer
            ...
            test_document_text = """The boys are running quickly through the fields. 
    They were hoping to catch butterflies, but the flies 
    kept bothering them. Happily, they enjoyed their day anyway!"""
            ...
            demonstration_tokens = ["The", "children", "were", "playing", "happily", "outside"]
            
            # Process with lowercase stemmer
            print("With lowercase stemming:")
            print(f"Original tokens: {demonstration_tokens}")
            try:
                lowercase_stemmed_tokens = lowercase_stemmer.stem_tokens(demonstration_tokens)
            ...
            print(f"Original tokens: {demonstration_tokens}")
            try:
                case_preserved_tokens = case_preserving_stemmer.stem_tokens(demonstration_tokens)
    ...
    def stem_words_from_args(word_list, preserve_case=False):
        ...
        # Create stemmer instance with appropriate case handling
        stemmer = PorterStemmer(to_lowercase=not preserve_case)
    ...
    # We use simple argument parsing for clarity and ease of modification
    ...
    # Process arguments
    # We check each possible option clearly and separately
